...
def generate_manifest(exclude=None):
    ...
    real module (routes, navItems, homeWidgets, etc.), but loaded via a
    ...
    homeWidgets is handled differently from the other exports: modules
    publish configurable units (a namespaced map, not a rendered list). The
    site decides what to do with them — either declaratively, via its own
    homeSections list (rendered by core's default HomePage.jsx), or fully,
    by exporting its own HomePage component that assembles the front page
    however it wants. See HomePage.jsx and core/README.md.
    """
    _check_route_collisions(exclude=exclude)
    enabled = set(_load_enabled_modules())

    imports, route_items, nav_items, provider_items, navbar_end_items, admin_card_items, user_section_items, home_widget_items = [], [], [], [], [], [], [], []
    site_home_sections = "[]"
    site_home_page = "null"
    ...
    if _has_site_content():
        ...
        site_home_sections = "siteModule.homeSections ?? []"
        site_home_page = "siteModule.HomePage ?? null"
    ...
    for module_dir in sorted(MODULES_DIR.iterdir()):
        ...
        home_widget_items.append(f"  {name}: ({name}Module.homeWidgets ?? {{}}),")
    ...
    lines += [
        ...
        *user_section_items,
        "];",
        "",
        "export const moduleHomeWidgets = {",
        *home_widget_items,
        "};",
        "",
        f"export const siteHomeSections = {site_home_sections};",
        "",
        f"export const siteHomePage = {site_home_page};",
        "",  # trailing newline
    ]
...
def generate_installed_modules(exclude=None):
    ...
    for entry in sorted(MODULES_DIR.iterdir()):
        ...
        required_settings_map[name] = data.get("required_settings", [])
    ...
    required_settings = []
    for name in ordered:
        for key in required_settings_map.get(name, []):
            if key not in seen_settings:
                seen_settings.add(key)
                required_settings.append(key)
...
def add(name):
    ...
    # Settings values are hand-maintained in core/backend/core/settings/base.py
    # (not generated) -- base.py raises ImproperlyConfigured at import time if
    # any of these are missing, so add them now before migrate/runserver below.
    required_settings = manifest.get("required_settings", [])
    if required_settings:
        print(f"\nNOTE: '{name}' requires these settings -- add them to "
              f"core/backend/core/settings/base.py (values from secrets.json "
              f"where appropriate) before continuing:")
